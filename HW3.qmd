...
```{r}
model2 <- read.table("trailuser.txt", header = TRUE, stringsAsFactors = TRUE)
model2$dayType <- as.factor(model2$dayType)
model2$season <- as.factor(model2$season)

contrasts(model2$dayType)
contrasts(model2$season)

additive_model <- lm(volume ~ avgtemp + precip + dayType + season, data = model2)
...
**I would favor reporting model 4**
